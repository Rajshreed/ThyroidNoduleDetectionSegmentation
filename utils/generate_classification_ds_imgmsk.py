...
for i in range(len(label_ds)):
    ...
    box[:,:,1] = mask[y1:y2, x1:x2,0]
    max_x = max(x2-x1,max_x) 
    max_y = max(y2-y1,max_y) 
    label = label_ds[i][1]

    #print(filename, label, box.shape)
    print (test_yes_count<200 and label=='1')
    if test_yes_count < 200 and label=='1' : #r valid_no_count < 300:
        path = './classification_ds/test_imgmsk/yes/'+filename
        test_yes_count+=1
            #print("copy valid 1",i, filename, label) #move valid - 1
    elif test_no_count < 164 and label=='0' :
        path = './classification_ds/test_imgmsk/no/'+filename
        test_no_count+=1
            #print("copy valid 0",i, filename, label) #move valid - 0
    elif valid_yes_count < 200 and label=='1' : #r valid_no_count < 300:
        path = './classification_ds/valid_imgmsk/yes/'+filename
        valid_yes_count+=1
            #print("copy valid 1",i, filename, label) #move valid - 1
    elif valid_no_count < 164 and label=='0' : 
        path = './classification_ds/valid_imgmsk/no/'+filename
        valid_no_count+=1
            #print("copy valid 0",i, filename, label) #move valid - 0
    elif label == '1':
        path = './classification_ds/train_imgmsk/yes/'+filename
            #print("copy train 1", i, filename, label) #move train - 1
    else:
        path = './classification_ds/train_imgmsk/no/'+filename
            #print("copy train 0", i, filename, label)#move train - 0
    print(i, test_yes_count, test_no_count, valid_yes_count, valid_no_count, path, box.shape)
